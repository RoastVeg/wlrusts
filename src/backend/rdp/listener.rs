 ...
 */
    ...
    fn _wlr_log(verbosity: wlr_log_importance, format: *const libc::c_char,
                _: ...);
    #[no_mangle]
    fn freerdp_listener_new() -> *mut freerdp_listener;
    #[no_mangle]
    ...
    fn wl_display_get_event_loop(display: *mut wl_display)
     -> *mut wl_event_loop;
    #[no_mangle]
    fn rdp_peer_init(client: *mut freerdp_peer, backend: *mut wlr_rdp_backend)
     -> libc::c_int;
}
...
unsafe extern "C" fn rdp_incoming_peer(mut listener: *mut freerdp_listener,
                                       mut client: *mut freerdp_peer)
 -> libc::c_int {
    let mut backend: *mut wlr_rdp_backend =
        (*listener).param4 as *mut wlr_rdp_backend;
    if rdp_peer_init(client, backend) < 0i32 {
        _wlr_log(WLR_ERROR,
                 b"[%s:%d] Error initializing incoming peer\x00" as *const u8
                     as *const libc::c_char,
                 b"../backend/rdp/listener.c\x00" as *const u8 as
                     *const libc::c_char, 10i32);
        return 0i32
    }
    return 1i32;
}
unsafe extern "C" fn rdp_listener_activity(mut fd: libc::c_int,
                                           mut mask: uint32_t,
                                           mut data: *mut libc::c_void)
 -> libc::c_int {
    let mut listener: *mut freerdp_listener = data as *mut freerdp_listener;
    if mask & WL_EVENT_READABLE as libc::c_int as libc::c_uint == 0 {
        return 0i32
    }
    if (*listener).CheckFileDescriptor.expect("non-null function pointer")(listener)
           == 0 {
        _wlr_log(WLR_ERROR,
                 b"[%s:%d] Failed to check FreeRDP file descriptor\x00" as
...
#[no_mangle]
pub unsafe extern "C" fn rdp_configure_listener(mut backend:
                                                    *mut wlr_rdp_backend)
 -> bool {
    (*backend).listener = freerdp_listener_new();
    if (*backend).listener.is_null() {
        _wlr_log(WLR_ERROR,
                 b"[%s:%d] Failed to allocate FreeRDP listener\x00" as
                     *const u8 as *const libc::c_char,
                 b"../backend/rdp/listener.c\x00" as *const u8 as
                     *const libc::c_char, 31i32);
        return 0i32 != 0
    }
    (*(*backend).listener).PeerAccepted =
        Some(rdp_incoming_peer as
                 unsafe extern "C" fn(_: *mut freerdp_listener,
                                      _: *mut freerdp_peer) -> libc::c_int);
    (*(*backend).listener).param4 = backend as *mut libc::c_void;
    if (*(*backend).listener).Open.expect("non-null function pointer")((*backend).listener,
                                                                       (*backend).address,
                                                                       (*backend).port
                                                                           as
                                                                           UINT16)
           == 0 {
        _wlr_log(WLR_ERROR,
                 b"[%s:%d] Failed to bind to RDP socket\x00" as *const u8 as
                     *const libc::c_char,
                 b"../backend/rdp/listener.c\x00" as *const u8 as
                     *const libc::c_char, 38i32);
        return 0i32 != 0
    }
    let mut rcount: libc::c_int = 0i32;
    let mut rfds: [*mut libc::c_void; 64] = [0 as *mut libc::c_void; 64];
    if (*(*backend).listener).GetFileDescriptor.expect("non-null function pointer")((*backend).listener,
                                                                                    rfds.as_mut_ptr(),
                                                                                    &mut rcount)
           == 0 {
        _wlr_log(WLR_ERROR,
                 b"[%s:%d] Failed to get FreeRDP file descriptors\x00" as
                     *const u8 as *const libc::c_char,
                 b"../backend/rdp/listener.c\x00" as *const u8 as
    ...
    while i < rcount {
        ...
        (*backend).listener_events[i as usize] =
            wl_event_loop_add_fd(event_loop, fd,
                                 WL_EVENT_READABLE as libc::c_int as uint32_t,
                                 Some(rdp_listener_activity as
                                          unsafe extern "C" fn(_: libc::c_int,
                                                               _: uint32_t,
                                                               _:
                                                                   *mut libc::c_void)
                                              -> libc::c_int),
                                 (*backend).listener as *mut libc::c_void);
        i += 1
    }
    while i < 64i32 {
        (*backend).listener_events[i as usize] = 0 as *mut wl_event_source;
        i += 1
    }
    return 1i32 != 0;
}
