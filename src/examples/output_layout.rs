 ...
 */
    ...
    #[no_mangle]
    fn wlr_output_layout_intersects(layout: *mut wlr_output_layout,
                                    reference: *mut wlr_output,
                                    target_lbox: *const wlr_box) -> bool;
    #[no_mangle]
    ...
    #[no_mangle]
    fn wlr_output_layout_output_at(layout: *mut wlr_output_layout,
...
unsafe extern "C" fn animate_cat(mut sample: *mut sample_state,
                                 mut output: *mut wlr_output) {
    let mut ts: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    clock_gettime(1i32, &mut ts);
    let mut ms: libc::c_long =
        (ts.tv_sec - (*sample).ts_last.tv_sec) * 1000i32 as libc::c_long +
            (ts.tv_nsec - (*sample).ts_last.tv_nsec) /
    ...
    let mut ur_collision: bool =
        wlr_output_layout_output_at((*sample).layout,
                                    ((*sample).x_offs +
                                         128i32 as libc::c_float) as
                                        libc::c_double,
                                    (*sample).y_offs as
                                        libc::c_double).is_null();
    let mut ul_collision: bool =
        wlr_output_layout_output_at((*sample).layout,
                                    (*sample).x_offs as libc::c_double,
                                    (*sample).y_offs as
                                        libc::c_double).is_null();
    let mut ll_collision: bool =
        wlr_output_layout_output_at((*sample).layout,
                                    (*sample).x_offs as libc::c_double,
                                    ((*sample).y_offs +
                                         128i32 as libc::c_float) as
                                        libc::c_double).is_null();
    let mut lr_collision: bool =
        wlr_output_layout_output_at((*sample).layout,
                                    ((*sample).x_offs +
                                         128i32 as libc::c_float) as
                                        libc::c_double,
                                    ((*sample).y_offs +
                                         128i32 as libc::c_float) as
                                        libc::c_double).is_null();
    if ur_collision as libc::c_int != 0 && ul_collision as libc::c_int != 0 &&
           ll_collision as libc::c_int != 0 &&
           lr_collision as libc::c_int != 0 {
        ...
        (*sample).x_offs = ((*l_output).x + 20i32) as libc::c_float;
        (*sample).y_offs = ((*l_output).y + 20i32) as libc::c_float
    } else if ur_collision as libc::c_int != 0 &&
                  ul_collision as libc::c_int != 0 {
        (*sample).y_vel =
            fabs((*sample).y_vel as libc::c_double) as libc::c_float
    } else if lr_collision as libc::c_int != 0 &&
                  ll_collision as libc::c_int != 0 {
        (*sample).y_vel =
            -fabs((*sample).y_vel as libc::c_double) as libc::c_float
    } else if ll_collision as libc::c_int != 0 &&
                  ul_collision as libc::c_int != 0 {
        (*sample).x_vel =
            fabs((*sample).x_vel as libc::c_double) as libc::c_float
    } else if ur_collision as libc::c_int != 0 &&
                  lr_collision as libc::c_int != 0 {
        (*sample).x_vel =
            -fabs((*sample).x_vel as libc::c_double) as libc::c_float
    } else {
        if ur_collision as libc::c_int != 0 ||
               lr_collision as libc::c_int != 0 {
            (*sample).x_vel =
                -fabs((*sample).x_vel as libc::c_double) as libc::c_float
        }
        if ul_collision as libc::c_int != 0 ||
               ll_collision as libc::c_int != 0 {
            (*sample).x_vel =
                fabs((*sample).x_vel as libc::c_double) as libc::c_float
        }
        if ul_collision as libc::c_int != 0 ||
               ur_collision as libc::c_int != 0 {
            (*sample).y_vel =
                fabs((*sample).y_vel as libc::c_double) as libc::c_float
        }
        if ll_collision as libc::c_int != 0 ||
               lr_collision as libc::c_int != 0 {
            (*sample).y_vel =
                -fabs((*sample).y_vel as libc::c_double) as libc::c_float
        }
...
unsafe extern "C" fn output_frame_notify(mut listener: *mut wl_listener,
    ...
    let mut box_0: wlr_box =
        {
            let mut init =
                wlr_box{x: (*sample).x_offs as libc::c_int,
                        y: (*sample).y_offs as libc::c_int,
                        width: 128i32,
                        height: 128i32,};
            init
        };
    if wlr_output_layout_intersects((*sample).layout, (*output).output,
        ...
        let mut local_x: libc::c_double = (*sample).x_offs as libc::c_double;
        let mut local_y: libc::c_double = (*sample).y_offs as libc::c_double;
        wlr_output_layout_output_coords((*sample).layout, (*output).output,
                                        &mut local_x, &mut local_y);
        wlr_render_texture((*sample).renderer, (*sample).cat_texture,
                           (*wlr_output).transform_matrix.as_mut_ptr() as
                               *const libc::c_float, local_x as libc::c_int,
                           local_y as libc::c_int, 1.0f32);
    }
...
unsafe extern "C" fn keyboard_key_notify(mut listener: *mut wl_listener,
    ...
    while i < nsyms {
		...
		// Also, key repeat
        let mut delta: libc::c_int = 75i32;
        if (*event).state as libc::c_uint ==
               WLR_KEY_PRESSED as libc::c_int as libc::c_uint {
            match sym {
                65361 => {
                    update_velocities(sample, -delta as libc::c_float,
        ...
        0 => {
            ...
            if context.is_null() {
                _wlr_log(WLR_ERROR,
                         ...
                         b"../examples/output-layout.c\x00" as *const u8 as
    ...
    let mut state: sample_state =
        {
            let mut init =
                sample_state{display: display,
                             ...
                             x_offs: 0.,
                             y_offs: 0.,
                             x_vel: 500i32 as libc::c_float,
                             y_vel: 500i32 as libc::c_float,
                             ts_last: timespec{tv_sec: 0, tv_nsec: 0,},};
            init
        };
    state.layout = wlr_output_layout_create();
    clock_gettime(1i32, &mut state.ts_last);
