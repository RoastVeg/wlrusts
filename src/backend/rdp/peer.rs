 ...
 */
    ...
    fn _wlr_log(verbosity: wlr_log_importance, format: *const libc::c_char,
                _: ...);
    #[no_mangle]
    fn Stream_New(buffer: *mut BYTE, size: size_t) -> *mut wStream;
    #[no_mangle]
    fn Stream_Free(s: *mut wStream, bFreeBuffer: BOOL);
    #[no_mangle]
    fn wlr_rdp_output_create(backend: *mut wlr_rdp_backend,
                             context: *mut wlr_rdp_peer_context,
                             width: libc::c_uint, height: libc::c_uint)
     -> *mut wlr_rdp_output;
    #[no_mangle]
    fn wlr_rdp_pointer_create(backend: *mut wlr_rdp_backend,
                              context: *mut wlr_rdp_peer_context)
     -> *mut wlr_rdp_input_device;
    #[no_mangle]
    fn wlr_rdp_keyboard_create(backend: *mut wlr_rdp_backend,
 ...
 */
    #[no_mangle]
    fn freerdp_peer_free(client: *mut freerdp_peer);
    #[no_mangle]
    fn freerdp_peer_context_free(client: *mut freerdp_peer);
    #[no_mangle]
    fn freerdp_peer_context_new(client: *mut freerdp_peer) -> BOOL;
    #[no_mangle]
    fn nsc_context_set_pixel_format(context: *mut NSC_CONTEXT,
                                    pixel_format: UINT32) -> BOOL;
    #[no_mangle]
    fn nsc_context_reset(context: *mut NSC_CONTEXT, width: UINT32,
                         height: UINT32) -> BOOL;
    #[no_mangle]
    fn nsc_context_new() -> *mut NSC_CONTEXT;
    #[no_mangle]
    fn nsc_context_free(context: *mut NSC_CONTEXT);
    #[no_mangle]
    fn GetVirtualKeyCodeFromVirtualScanCode(scancode: DWORD,
                                            dwKeyboardType: DWORD) -> DWORD;
    #[no_mangle]
    fn GetKeycodeFromVirtualKeyCode(keycode: DWORD, dwFlags: DWORD) -> DWORD;
    #[no_mangle]
    fn rfx_context_set_pixel_format(context: *mut RFX_CONTEXT,
                                    pixel_format: UINT32);
    #[no_mangle]
    fn rfx_context_reset(context: *mut RFX_CONTEXT, width: UINT32,
                         height: UINT32) -> BOOL;
    #[no_mangle]
    fn rfx_context_new(encoder: BOOL) -> *mut RFX_CONTEXT;
    #[no_mangle]
    fn rfx_context_free(context: *mut RFX_CONTEXT);
    #[no_mangle]
...
pub type wlr_rdp_peer_flags = libc::c_uint;
pub const RDP_PEER_OUTPUT_ENABLED: wlr_rdp_peer_flags = 2;
pub const RDP_PEER_ACTIVATED: wlr_rdp_peer_flags = 1;
unsafe extern "C" fn xf_peer_capabilities(mut client: *mut freerdp_peer)
 -> BOOL {
    return 1i32;
}
unsafe extern "C" fn xf_peer_post_connect(mut client: *mut freerdp_peer)
 -> BOOL {
    return 1i32;
}
unsafe extern "C" fn xf_peer_activate(mut client: *mut freerdp_peer) -> BOOL {
    let mut context: *mut wlr_rdp_peer_context =
        (*client).context as *mut wlr_rdp_peer_context;
    let mut backend: *mut wlr_rdp_backend = (*context).backend;
    let mut settings: *mut rdpSettings = (*client).settings;
    if (*settings).SurfaceCommandsEnabled == 0 {
        _wlr_log(WLR_ERROR,
                 b"[%s:%d] RDP peer does not support SurfaceCommands\x00" as
                     *const u8 as *const libc::c_char,
                 b"../backend/rdp/peer.c\x00" as *const u8 as
                     *const libc::c_char, 28i32);
        return 0i32
    }
    (*context).output =
        wlr_rdp_output_create(backend, context,
                              (*settings).DesktopWidth as libc::c_int as
                                  libc::c_uint,
                              (*settings).DesktopHeight as libc::c_int as
                                  libc::c_uint);
    if (*context).output.is_null() {
        _wlr_log(WLR_ERROR,
                 b"[%s:%d] Failed to allcoate output for RDP peer\x00" as
                     *const u8 as *const libc::c_char,
                 b"../backend/rdp/peer.c\x00" as *const u8 as
    ...
    rfx_context_reset((*context).rfx_context,
                      (*(*context).output).wlr_output.width as UINT32,
                      (*(*context).output).wlr_output.height as UINT32);
    nsc_context_reset((*context).nsc_context,
                      (*(*context).output).wlr_output.width as UINT32,
                      (*(*context).output).wlr_output.height as UINT32);
    if (*context).flags & RDP_PEER_ACTIVATED as libc::c_int as libc::c_uint !=
           0 {
        return 1i32
    }
    ...
    if (*context).pointer.is_null() {
        _wlr_log(WLR_ERROR,
                 b"[%s:%d] Failed to allocate pointer for RDP peer\x00" as
                     *const u8 as *const libc::c_char,
                 b"../backend/rdp/peer.c\x00" as *const u8 as
    ...
    let mut pointer_system: POINTER_SYSTEM_UPDATE =
        POINTER_SYSTEM_UPDATE{type_0: 0,};
    let mut pointer: *mut rdpPointerUpdate = (*(*client).update).pointer;
    pointer_system.type_0 = 0i32 as UINT32;
    (*pointer).PointerSystem.expect("non-null function pointer")((*client).context,
                                                                 &mut pointer_system);
    (*context).keyboard = wlr_rdp_keyboard_create(backend, settings);
    if (*context).keyboard.is_null() {
        _wlr_log(WLR_ERROR,
                 b"[%s:%d] Failed to allocate keyboard for RDP peer\x00" as
                     *const u8 as *const libc::c_char,
                 b"../backend/rdp/peer.c\x00" as *const u8 as
                     *const libc::c_char, 63i32);
        return 0i32
    }
    (*context).flags |= RDP_PEER_ACTIVATED as libc::c_int as libc::c_uint;
    return 1i32;
}
unsafe extern "C" fn xf_suppress_output(mut context: *mut rdpContext,
                                        mut allow: BYTE,
                                        mut area: *const RECTANGLE_16)
 -> libc::c_int {
    let mut peer_context: *mut wlr_rdp_peer_context =
        context as *mut wlr_rdp_peer_context;
    if allow != 0 {
        (*peer_context).flags |=
            RDP_PEER_OUTPUT_ENABLED as libc::c_int as libc::c_uint
    } else {
        (*peer_context).flags &=
            !(RDP_PEER_OUTPUT_ENABLED as libc::c_int) as libc::c_uint
    }
...
unsafe extern "C" fn xf_input_synchronize_event(mut input: *mut rdpInput,
                                                mut flags: UINT32)
 -> libc::c_int {
    let mut context: *mut wlr_rdp_peer_context =
        (*input).context as *mut wlr_rdp_peer_context;
    wlr_output_damage_whole(&mut (*(*context).output).wlr_output);
    return 1i32;
}
#[inline]
...
unsafe extern "C" fn xf_input_mouse_event(mut input: *mut rdpInput,
                                          mut flags: UINT16, mut x: UINT16,
                                          mut y: UINT16) -> libc::c_int {
    let mut context: *mut wlr_rdp_peer_context =
        (*input).context as *mut wlr_rdp_peer_context;
    let mut wlr_device: *mut wlr_input_device =
        &mut (*(*context).pointer).wlr_input_device;
    let mut pointer: *mut wlr_pointer = (*wlr_device).c2rust_unnamed.pointer;
    let mut frame: bool = 0i32 != 0;
    ...
    if flags as libc::c_int & 0x800i32 != 0 {
        ...
        event.time_msec = timespec_to_msec(&mut now) as uint32_t;
        event.x =
            x as libc::c_int as libc::c_double /
                (*(*context).output).wlr_output.width as libc::c_double;
        event.y =
            y as libc::c_int as libc::c_double /
                (*(*context).output).wlr_output.height as libc::c_double;
        wlr_signal_emit_safe(&mut (*pointer).events.motion_absolute,
                             &mut event as
                                 *mut wlr_event_pointer_motion_absolute as
                                 *mut libc::c_void);
        frame = 1i32 != 0
    }
    let mut button: uint32_t = 0i32 as uint32_t;
    if flags as libc::c_int & 0x1000i32 != 0 {
        button = 0x110i32 as uint32_t
    } else if flags as libc::c_int & 0x2000i32 != 0 {
        button = 0x111i32 as uint32_t
    } else if flags as libc::c_int & 0x4000i32 != 0 {
        button = 0x112i32 as uint32_t
    }
    if button != 0 {
        let mut event_0: wlr_event_pointer_button =
            {
                let mut init =
        ...
        event_0.device = wlr_device;
        event_0.time_msec = timespec_to_msec(&mut now) as uint32_t;
        event_0.button = button;
        event_0.state =
            if flags as libc::c_int & 0x8000i32 != 0 {
                WLR_BUTTON_PRESSED as libc::c_int
            } else { WLR_BUTTON_RELEASED as libc::c_int } as wlr_button_state;
        wlr_signal_emit_safe(&mut (*pointer).events.button,
                             &mut event_0 as *mut wlr_event_pointer_button as
                                 *mut libc::c_void);
        frame = 1i32 != 0
    }
    if flags as libc::c_int & 0x200i32 != 0 {
        let mut value: libc::c_double =
            -(flags as libc::c_int & 0xffi32) as libc::c_double / 120.0f64;
        if flags as libc::c_int & 0x100i32 != 0 { value = -value }
        let mut event_1: wlr_event_pointer_axis =
            {
                let mut init =
        ...
        event_1.device = &mut (*(*context).pointer).wlr_input_device;
        event_1.time_msec = timespec_to_msec(&mut now) as uint32_t;
        event_1.source = WLR_AXIS_SOURCE_WHEEL;
        event_1.orientation = WLR_AXIS_ORIENTATION_VERTICAL;
        event_1.delta = value;
        event_1.delta_discrete = value as int32_t;
        wlr_signal_emit_safe(&mut (*pointer).events.axis,
                             &mut event_1 as *mut wlr_event_pointer_axis as
                                 *mut libc::c_void);
        frame = 1i32 != 0
    }
    if frame {
        wlr_signal_emit_safe(&mut (*pointer).events.frame,
                             pointer as *mut libc::c_void);
    }
    return 1i32;
}
unsafe extern "C" fn xf_input_extended_mouse_event(mut input: *mut rdpInput,
                                                   mut flags: UINT16,
                                                   mut x: UINT16,
                                                   mut y: UINT16)
 -> libc::c_int {
...
unsafe extern "C" fn xf_input_keyboard_event(mut input: *mut rdpInput,
                                             mut flags: UINT16,
                                             mut code: UINT16)
 -> libc::c_int {
    let mut context: *mut wlr_rdp_peer_context =
        (*input).context as *mut wlr_rdp_peer_context;
    let mut wlr_device: *mut wlr_input_device =
        &mut (*(*context).keyboard).wlr_input_device;
    ...
    let mut notify: bool = 0i32 != 0;
    let mut state: wlr_key_state = WLR_KEY_RELEASED;
    if flags as libc::c_int & 0x4000i32 != 0 {
        state = WLR_KEY_PRESSED;
        notify = 1i32 != 0
    } else if flags as libc::c_int & 0x8000i32 != 0 {
        state = WLR_KEY_RELEASED;
        notify = 1i32 != 0
    }
    if notify {
        let mut full_code: uint32_t = code as uint32_t;
        if flags as libc::c_int & 0x100i32 != 0 {
            full_code |= 0x100i32 as libc::c_uint
        }
        let mut vk_code: uint32_t =
            GetVirtualKeyCodeFromVirtualScanCode(full_code, 4i32 as DWORD);
        if flags as libc::c_int & 0x100i32 != 0 {
            vk_code |= 0x100i32 as USHORT as libc::c_uint
        }
        let mut scan_code: uint32_t =
            GetKeycodeFromVirtualKeyCode(vk_code, 0x2i32 as DWORD);
        ...
        event.time_msec = timespec_to_msec(&mut now) as uint32_t;
        event.keycode = scan_code.wrapping_sub(8i32 as libc::c_uint);
        event.state = state;
        event.update_state = 1i32 != 0;
        wlr_keyboard_notify_key(keyboard, &mut event);
    }
    return 1i32;
}
unsafe extern "C" fn xf_input_unicode_keyboard_event(mut input: *mut rdpInput,
                                                     mut flags: UINT16,
                                                     mut code: UINT16)
 -> libc::c_int {
    _wlr_log(WLR_DEBUG,
             b"[%s:%d] Unhandled RDP unicode keyboard event (flags:0x%X code:0x%X)\n\x00"
                 as *const u8 as *const libc::c_char,
             b"../backend/rdp/peer.c\x00" as *const u8 as *const libc::c_char,
             222i32, flags as libc::c_int, code as libc::c_int);
    return 1i32;
}
unsafe extern "C" fn rdp_client_activity(mut fd: libc::c_int,
                                         mut mask: uint32_t,
                                         mut data: *mut libc::c_void)
 -> libc::c_int {
    let mut client: *mut freerdp_peer = data as *mut freerdp_peer;
    if (*client).CheckFileDescriptor.expect("non-null function pointer")(client)
           == 0 {
        _wlr_log(WLR_ERROR,
                 b"[%s:%d] Unable to check client file descriptor for %p\x00"
                     as *const u8 as *const libc::c_char,
                 b"../backend/rdp/peer.c\x00" as *const u8 as
                     *const libc::c_char, 230i32, client);
        freerdp_peer_context_free(client);
        freerdp_peer_free(client);
    }
    return 0i32;
}
unsafe extern "C" fn rdp_peer_context_new(mut client: *mut freerdp_peer,
                                          mut context:
                                              *mut wlr_rdp_peer_context)
 -> libc::c_int {
    (*context).peer = client;
    (*context).flags = RDP_PEER_OUTPUT_ENABLED as libc::c_int as uint32_t;
    (*context).rfx_context = rfx_context_new(1i32);
    if (*context).rfx_context.is_null() { return 0i32 }
    (*(*context).rfx_context).mode = RLGR3;
    (*(*context).rfx_context).width =
        (*(*client).settings).DesktopWidth as UINT16;
    (*(*context).rfx_context).height =
        (*(*client).settings).DesktopHeight as UINT16;
    rfx_context_set_pixel_format((*context).rfx_context,
                                 (32i32 << 24i32 | 4i32 << 16i32 |
                                      8i32 << 12i32 | 8i32 << 8i32 |
                                      8i32 << 4i32 | 8i32) as UINT32);
    (*context).nsc_context = nsc_context_new();
    if (*context).nsc_context.is_null() {
    ...
    nsc_context_set_pixel_format((*context).nsc_context,
                                 (32i32 << 24i32 | 4i32 << 16i32 |
                                      8i32 << 12i32 | 8i32 << 8i32 |
                                      8i32 << 4i32 | 8i32) as UINT32);
    (*context).encode_stream = Stream_New(0 as *mut BYTE, 65536i32 as size_t);
    if (*context).encode_stream.is_null() {
...
unsafe extern "C" fn rdp_peer_context_free(mut client: *mut freerdp_peer,
                                           mut context:
                                               *mut wlr_rdp_peer_context) {
    if context.is_null() { return }
    let mut i: libc::c_int = 0i32;
    while i < 64i32 {
        if !(*context).events[i as usize].is_null() {
            wl_event_source_remove((*context).events[i as usize]);
        }
        i += 1
    }
    if (*context).flags & RDP_PEER_ACTIVATED as libc::c_int as libc::c_uint !=
           0 {
        wlr_output_destroy(&mut (*(*context).output).wlr_output);
        wlr_input_device_destroy(&mut (*(*context).pointer).wlr_input_device);
        wlr_input_device_destroy(&mut (*(*context).keyboard).wlr_input_device);
    }
    wl_list_remove(&mut (*context).link);
    wlr_output_destroy(&mut (*(*context).output).wlr_output);
    Stream_Free((*context).encode_stream, 1i32);
    nsc_context_free((*context).nsc_context);
    rfx_context_free((*context).rfx_context);
    free((*context).rfx_rects as *mut libc::c_void);
}
// ms
#[no_mangle]
pub unsafe extern "C" fn rdp_peer_init(mut client: *mut freerdp_peer,
                                       mut backend: *mut wlr_rdp_backend)
 -> libc::c_int {
    let mut rcount: libc::c_int = 0;
    let mut rfds: [*mut libc::c_void; 64] = [0 as *mut libc::c_void; 64];
    let mut event_loop: *mut wl_event_loop = 0 as *mut wl_event_loop;
    let mut i: libc::c_int = 0;
    (*client).ContextSize =
        ::std::mem::size_of::<wlr_rdp_peer_context>() as libc::c_ulong;
    (*client).ContextNew =
        ::std::mem::transmute::<Option<unsafe extern "C" fn(_:
                                                            ...
                                                            _:
                                                                *mut wlr_rdp_peer_context)
                                           -> libc::c_int>,
                                psPeerContextNew>(Some(rdp_peer_context_new as
    ...
    (*client).ContextFree =
        ::std::mem::transmute::<Option<unsafe extern "C" fn(_:
                                                            ...
                                                            _:
                                                                *mut wlr_rdp_peer_context)
                                           -> ()>,
                                psPeerContextFree>(Some(rdp_peer_context_free
                                                            as
                                                            unsafe extern "C" fn(_:
                                                                                     *mut freerdp_peer,
                                                                                 _:
    ...
    (*(*client).settings).NlaSecurity = 0i32;
    if (*client).Initialize.expect("non-null function pointer")(client) == 0 {
        _wlr_log(WLR_ERROR,
                 b"[%s:%d] Failed to initialize FreeRDP peer\x00" as *const u8
                     as *const libc::c_char,
                 b"../backend/rdp/peer.c\x00" as *const u8 as
                     *const libc::c_char, 310i32);
    } else {
        (*(*client).settings).OsMajorType = 0x4i32 as UINT32;
        (*(*client).settings).OsMinorType = 0x8i32 as UINT32;
        (*(*client).settings).ColorDepth = 32i32 as UINT32;
        (*(*client).settings).RefreshRect = 1i32;
        (*(*client).settings).RemoteFxCodec = 1i32;
        (*(*client).settings).NSCodec = 1i32;
        (*(*client).settings).FrameMarkerCommandEnabled = 1i32;
        (*(*client).settings).SurfaceFrameMarkerEnabled = 1i32;
        (*client).Capabilities =
            Some(xf_peer_capabilities as
                     unsafe extern "C" fn(_: *mut freerdp_peer) -> BOOL);
        (*client).PostConnect =
            Some(xf_peer_post_connect as
                     unsafe extern "C" fn(_: *mut freerdp_peer) -> BOOL);
        (*client).Activate =
            Some(xf_peer_activate as
                     unsafe extern "C" fn(_: *mut freerdp_peer) -> BOOL);
        (*(*client).update).SuppressOutput =
            ::std::mem::transmute::<Option<unsafe extern "C" fn(_:
                                                                    *mut rdpContext,
                                                                _: BYTE,
                                                                _:
                                                                    *const RECTANGLE_16)
                                               -> libc::c_int>,
                                    pSuppressOutput>(Some(xf_suppress_output
                                                              as
                                                              unsafe extern "C" fn(_:
                                                                                       *mut rdpContext,
                                                                                   _:
                                                                  ...
                                                                  ->
                                                                      libc::c_int));
        (*(*client).input).SynchronizeEvent =
            Some(xf_input_synchronize_event as
                     unsafe extern "C" fn(_: *mut rdpInput, _: UINT32)
                         -> libc::c_int);
        (*(*client).input).MouseEvent =
            Some(xf_input_mouse_event as
                     unsafe extern "C" fn(_: *mut rdpInput, _: UINT16,
                                          _: UINT16, _: UINT16)
                         -> libc::c_int);
        (*(*client).input).ExtendedMouseEvent =
            Some(xf_input_extended_mouse_event as
                     unsafe extern "C" fn(_: *mut rdpInput, _: UINT16,
                                          _: UINT16, _: UINT16)
                         -> libc::c_int);
        (*(*client).input).KeyboardEvent =
            Some(xf_input_keyboard_event as
                     unsafe extern "C" fn(_: *mut rdpInput, _: UINT16,
                                          _: UINT16) -> libc::c_int);
        (*(*client).input).UnicodeKeyboardEvent =
            Some(xf_input_unicode_keyboard_event as
                     unsafe extern "C" fn(_: *mut rdpInput, _: UINT16,
                                          _: UINT16) -> libc::c_int);
        rcount = 0i32;
        rfds = [0 as *mut libc::c_void; 64];
        if (*client).GetFileDescriptor.expect("non-null function pointer")(client,
                                                                           rfds.as_mut_ptr(),
                                                                           &mut rcount)
               == 0 {
            _wlr_log(WLR_ERROR,
                     b"[%s:%d] Unable to retrieve client file descriptors\x00"
                         as *const u8 as *const libc::c_char,
                     b"../backend/rdp/peer.c\x00" as *const u8 as
                         *const libc::c_char, 338i32);
        } else {
            ...
            while i < rcount {
                let mut fd: libc::c_int =
                    rfds[i as usize] as libc::c_long as libc::c_int;
                (*peer_context).events[i as usize] =
                    wl_event_loop_add_fd(event_loop, fd,
                                         WL_EVENT_READABLE as libc::c_int as
                                             uint32_t,
                                         Some(rdp_client_activity as
                                                  unsafe extern "C" fn(_:
                                                                           libc::c_int,
                                                                       _:
                                                                           uint32_t,
            ...
            while i < 64i32 {
                (*peer_context).events[i as usize] =
                    0 as *mut wl_event_source;
                i += 1
            }
            wl_list_insert(&mut (*backend).clients,
                           &mut (*peer_context).link);
            return 0i32
        }
    }
    (*client).Close.expect("non-null function pointer")(client);
    return -1i32;
}
