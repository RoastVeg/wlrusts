 ...
 */
    ...
    #[no_mangle]
    fn rdp_configure_listener(backend: *mut wlr_rdp_backend) -> bool;
    #[no_mangle]
...
// ms
#[no_mangle]
pub unsafe extern "C" fn rdp_backend_from_backend(mut wlr_backend:
                                                      *mut wlr_backend)
 -> *mut wlr_rdp_backend {
    if wlr_backend_is_rdp(wlr_backend) as libc::c_int != 0 {
    } else {
        __assert_fail(b"wlr_backend_is_rdp(wlr_backend)\x00" as *const u8 as
                          *const libc::c_char,
                      b"../backend/rdp/backend.c\x00" as *const u8 as
                          *const libc::c_char, 14i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 71],
                                                &[libc::c_char; 71]>(b"struct wlr_rdp_backend *rdp_backend_from_backend(struct wlr_backend *)\x00")).as_ptr());
    };
    return wlr_backend as *mut wlr_rdp_backend;
}
unsafe extern "C" fn backend_start(mut wlr_backend: *mut wlr_backend)
 -> bool {
    let mut backend: *mut wlr_rdp_backend =
        rdp_backend_from_backend(wlr_backend);
    if (*backend).listener.is_null() {
    } else {
        __assert_fail(b"backend->listener == NULL\x00" as *const u8 as
                          *const libc::c_char,
                      b"../backend/rdp/backend.c\x00" as *const u8 as
                          *const libc::c_char, 21i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 42],
                                                &[libc::c_char; 42]>(b"_Bool backend_start(struct wlr_backend *)\x00")).as_ptr());
    };
    _wlr_log(WLR_INFO,
             b"[%s:%d] Starting RDP backend\x00" as *const u8 as
                 *const libc::c_char,
             b"../backend/rdp/backend.c\x00" as *const u8 as
                 *const libc::c_char, 22i32);
    if !rdp_configure_listener(backend) { return 0i32 != 0 }
    return 1i32 != 0;
}
#[no_mangle]
pub unsafe extern "C" fn wlr_rdp_backend_set_address(mut wlr_backend:
                                                         *mut wlr_backend,
                                                     mut address:
                                                         *const libc::c_char) {
    let mut backend: *mut wlr_rdp_backend =
    ...
    if (*backend).listener.is_null() {
        ...
        __assert_fail(b"backend->listener == NULL\x00" as *const u8 as
                      ...
                      (*::std::mem::transmute::<&[u8; 69],
                                                &[libc::c_char; 69]>(b"void wlr_rdp_backend_set_address(struct wlr_backend *, const char *)\x00")).as_ptr());
    };
    (*backend).address = strdup(address);
}
#[no_mangle]
pub unsafe extern "C" fn wlr_rdp_backend_set_port(mut wlr_backend:
                                                      *mut wlr_backend,
                                                  mut port: libc::c_int) {
    let mut backend: *mut wlr_rdp_backend =
        rdp_backend_from_backend(wlr_backend);
    if (*backend).listener.is_null() {
        ...
        __assert_fail(b"backend->listener == NULL\x00" as *const u8 as
                          *const libc::c_char,
                      b"../backend/rdp/backend.c\x00" as *const u8 as
                          *const libc::c_char, 40i32 as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 57],
                                                &[libc::c_char; 57]>(b"void wlr_rdp_backend_set_port(struct wlr_backend *, int)\x00")).as_ptr());
    };
    (*backend).port = port;
}
unsafe extern "C" fn backend_destroy(mut wlr_backend: *mut wlr_backend) {
    ...
    let mut client: *mut wlr_rdp_peer_context =
        0 as *mut wlr_rdp_peer_context;
    client =
        ((*backend).clients.next as *mut libc::c_char).offset(-1616) as
            *mut wlr_rdp_peer_context;
    while &mut (*client).link as *mut wl_list !=
              &mut (*backend).clients as *mut wl_list {
        freerdp_peer_context_free((*client).peer);
        freerdp_peer_free((*client).peer);
        client =
            ((*client).link.next as *mut libc::c_char).offset(-1616) as
                *mut wlr_rdp_peer_context
    }
    ...
    free((*backend).address as *mut libc::c_void);
...
unsafe extern "C" fn handle_display_destroy(mut listener: *mut wl_listener,
    ...
    let mut backend: *mut wlr_rdp_backend =
        (listener as *mut libc::c_char).offset(-152) as *mut wlr_rdp_backend;
    backend_destroy(&mut (*backend).backend);
}
#[no_mangle]
pub unsafe extern "C" fn wlr_rdp_backend_create(mut display: *mut wl_display,
 ...
 -> *mut wlr_backend {
    _wlr_log(WLR_INFO,
             b"[%s:%d] Creating RDP backend\x00" as *const u8 as
                 *const libc::c_char,
             b"../backend/rdp/backend.c\x00" as *const u8 as
                 *const libc::c_char, 89i32);
    let mut backend: *mut wlr_rdp_backend =
        calloc(1i32 as libc::c_ulong,
               ::std::mem::size_of::<wlr_rdp_backend>() as libc::c_ulong) as
            *mut wlr_rdp_backend;
    if backend.is_null() {
        _wlr_log(WLR_ERROR,
                 b"[%s:%d] Failed to allocate wlr_rdp_backend\x00" as
    ...
    (*backend).address =
        strdup(b"127.0.0.1\x00" as *const u8 as *const libc::c_char);
    (*backend).port = 3389i32;
    wl_list_init(&mut (*backend).clients);
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn wlr_backend_is_rdp(mut backend: *mut wlr_backend)
 -> bool {
